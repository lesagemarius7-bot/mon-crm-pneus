...
import { listDealsByStageAction } from "@/lib/actions/deals";
...
export function PipelineByStageChart({ data }: { data: PipelineStageDatum[] }) {
  ...
  async function handleBarClick(entry: PipelineStageDatum) {
    if (entry.count === 0) return;
    setOpen(true);
    setLoading(true);
    setTitle(`${entry.count} deal${entry.count > 1 ? "s" : ""} — Étape « ${entry.name} »`);

    const deals = await listDealsByStageAction(entry.stageId);
    setItems(
      deals.map((deal) => ({
        id: deal.id,
        title: deal.name,
        ...
        status: {
          ...
          variant: deal.stage.isWon ? "default" : deal.stage.isLost ? "secondary" : "outline",
        },
        company: deal.company,
        owner: deal.owner,
        date: deal.closedAt ?? deal.expectedCloseDate ?? deal.createdAt,
        ...
        <BarChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 8 }}>
          ...
          <Tooltip
            ...
            formatter={(value, _name, item) => [
              ...
              `${item.payload.count} deal${item.payload.count > 1 ? "s" : ""} — cliquer pour le détail`,
            ]}
          />
